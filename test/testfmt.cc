...
class Benchmark {
 private:
  time_point<steady_clock, nanoseconds> t0;
  double elapsedTime;
  std::string msg;
 ...
 public:
  ...
  void start() { t0 = time_point_cast<nanoseconds>(steady_clock::now()); }

  void end() {
    time_point<steady_clock, nanoseconds> t1 =
        time_point_cast<nanoseconds>(steady_clock::now());
    elapsedTime += (t1 - t0).count();
  }

  constexpr double elapsed() { return elapsedTime; }
  void display() const { print("{} elapsed time: {}\n", msg, elapsedTime); }
...
constexpr void testboth(T data) {
  ...
  Benchmark::benchmark("fmtlib format", n,
                       bind(test_format<vector<char>, T>, out, data));
  Benchmark::benchmark("sprintf format", n, bind(test_sprintf<T>, dest, data));
  Benchmark::benchmark("Grail float", n, bind(_formatgrl5<T>, dest, data));
  print("{}\n", data);
}
...
constexpr void testAll(T data) {
  ...
  Benchmark::benchmark("fmtlib format", n,
                       bind(test_format<vector<char>, T>, out, data));
  Benchmark::benchmark("sprintf format", n, bind(test_sprintf<T>, dest, data));
  Benchmark::benchmark("grail format RtoL 1 char", n,
                       bind(_formatgrl<T>, dest, data));
  Benchmark::benchmark("grail format RtoL 2 char", n,
                       bind(_formatgrl2<T>, dest, data));
  Benchmark::benchmark("grail format RtoL 4 char pairs", n,
                       bind(_formatgrl3<T>, dest, data));
  Benchmark::benchmark("grail format RtoL 4 char", n,
